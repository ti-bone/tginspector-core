/*
 * sql/writeMessage.cpp
 * Core of TgInspector, namespace for working with DB, function for writing message in DB, do not sell\post it without permission!
...
void Sql::writeMessage(const std::string& chat_id, const std::string& chat_name, const std::string& message_id,
                       const std::string& message_timestamp, const std::string& sender_id, const std::string& first_name,
                       const std::string& last_name, const std::string& text, const std::string& file_id, const std::string& file_type)
{
    int dbs = 0;
    ...
    if (cfg.get<bool>("db.postgresql.enabled")) {
        try {
            ...
            spdlog::debug("<PostgreSQL> Executing write message...");
            ...
            // Execute SQL query
            transaction.exec("insert into " + cfg.get<std::string>("db.postgresql.messages_table") +
                             " values ('" +
                             transaction.esc(UFP) +
                             ...
                             "', '" + transaction.esc(file_id) +
                             "', '" + transaction.esc(file_type) + "')");

            spdlog::debug("<PostgreSQL> INSERT Query executed in DB: {}, Table: {} [Server: {}, Port: {}].",
                            pqconn.dbname(), cfg.get<std::string>("db.postgresql.messages_table"),
            ...
            spdlog::debug("<PostgreSQL> Transaction committed in DB: {}, Table: {} [Server: {}, Port: {}].",
                          pqconn.dbname(), cfg.get<std::string>("db.postgresql.messages_table"),
                          pqconn.hostname(), pqconn.port());
            spdlog::debug("<PostgreSQL> ID of message, that was written: {}.", message_id);
            spdlog::debug("<PostgreSQL, Perf> Writing message in DB took {} seconds.", global_sw);
            dbs++;
    ...
    if (cfg.get<bool>("db.mysql.enabled")) {
        ...
        try {
            ...
            spdlog::debug("<MySQL> Executing write message...");
            ...
            // write data in messages table
            // prepare query
            pstmt = con->prepareStatement("INSERT INTO " + cfg.get<std::string>("db.mysql.messages_table") +
                                          "(`fp`, `chat_id`, `chat_name`, `message_id`, `message_timestamp`, `sender_id`, `first_name`,"
                                          " `last_name`, `text`, `file_id`, `file_type`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ");
            ...
            spdlog::debug("<MySQL> INSERT Query executed in DB: {}, Table: {} [Server: {}].", con->getSchema().asStdString(),
                                                                                              cfg.get<std::string>("db.mysql.messages_table"),
                                                                                              cfg.get<std::string>("db.mysql.host"));
            spdlog::debug("<MySQL> ID of message, that was written: {}.", message_id);
            spdlog::debug("<MySQL, Perf> Executing INSERT Query took {} seconds.", query_sw);
            spdlog::debug("<MySQL, Perf> Writing message in DB took {} seconds.", global_sw);
            delete pstmt;
            delete con;
            dbs++;
    ...
    spdlog::debug("<Perf> Writing message in {} DB(s) took {} seconds.", dbs, total_sw);
}